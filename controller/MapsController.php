...
class MapsController extends Controller{
	...
	function index(){
		// Récupérations des maps

		$this->loadModel('Maps');
		$d['maps'] = $this->Maps->find(array(
			'ordre' => array('champ' => 'nom','sens' => 'ASC')
		));
		if(empty($d['maps'])){
			$this->e404('Il n\'y a pas encore de maps enregistrées');
		}
		$this->set($d);
	}
	...
	function user_edit($id = null){
		...
		if($this->request->data){
			...
			$this->Session->setFlash('La map a bien été modifiée','success',2);
			$this->redirect('');
		}
		if($id){
